...
for (let i = 1; i <= WORKER_COUNT; i++) {
  ...
  connection.on("error", (error) => {
    logger.error(`Worker cloud ${i} - Redis connection error:`, error);
  });

  connection.on("connect", () => {
    logger.info(`Worker cloud ${i} connected to Redis`);
  });

  connection.on("reconnecting", () => {
    logger.info(`Worker cloud ${i} - Reconnecting to Redis`);
  });

  connection.on("close", () => {
    logger.warn(`Worker cloud ${i} - Redis connection closed`);
  });
